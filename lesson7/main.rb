...
class Main
  TRAIN_TYPES = [PassangerTrain, CargoTrain].freeze
  WAGON_TYPES = [PassangerWagon, CargoWagon].freeze
  ...
  def show
    @trains.each do |train|
      ...
      num = wagons.count
      puts num.to_s
      train.wagon_search do |wagon|
        wagon = wagon.busy_volume
        puts "Занятых мест в вагоне #{wagon}"
      end
      train.wagon_search do |wagon|
        wagon = wagon.free_volume
        puts "Свободных мест в вагоне #{wagon}"
      end
    end
